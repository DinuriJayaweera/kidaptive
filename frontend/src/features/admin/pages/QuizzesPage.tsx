...
export default function QuizzesPage() {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ["quizQuestions", page, ageGroupFilter, categoryFilter, difficultyFilter, searchTerm],
        queryFn: () =>
            getQuestions({
                page,
                limit: 8,
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, payload }: { id: string; payload: Partial<QuizQuestion> }) =>
    ...
    const handleOpenEdit = (q: QuizQuestion) => { setIsViewMode(false); setEditingQuestion(q); setIsModalOpen(true); };
    const handleOpenView = (q: QuizQuestion) => { setIsViewMode(true); setEditingQuestion(q); setIsModalOpen(true); };
    const handleDelete = (id: string) => { setDeleteTargetId(id); };
    const handleSave = (formData: Partial<QuizQuestion>) => {
