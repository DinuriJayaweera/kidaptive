...
import { getQuestions, getStats, createQuestion, updateQuestion, deleteQuestion } from "../api/dailyQuestApi";
import type { DailyQuestQuestion } from "../api/dailyQuestApi";
...
export default function DailyQuestQuestionsPage() {
    ...
    const [editingQuestion, setEditingQuestion] = useState<DailyQuestQuestion | null>(null);
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ["dailyQuestQuestions", page, ageGroupFilter, categoryFilter, difficultyFilter, searchTerm],
        queryFn: () =>
            getQuestions({
                page,
                limit: 8,
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["dailyQuestQuestions"] });
            queryClient.invalidateQueries({ queryKey: ["dailyQuestStats"] });
            setIsModalOpen(false);
        },
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, payload }: { id: string; payload: Partial<DailyQuestQuestion> }) =>
    ...
    const handleOpenEdit = (q: DailyQuestQuestion) => { setIsViewMode(false); setEditingQuestion(q); setIsModalOpen(true); };
    const handleOpenView = (q: DailyQuestQuestion) => { setIsViewMode(true); setEditingQuestion(q); setIsModalOpen(true); };
    ...
    return (
        <Box sx={{ width: "100%", fontFamily: "'Poppins', sans-serif" }}>
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", mb: { xs: 2.5, md: 3 }, flexWrap: "wrap", gap: 2 }}>
                <Box>
                    ...
                    <Typography sx={{ color: "var(--text-secondary)", fontSize: { xs: "0.875rem", md: "0.95rem" }, fontFamily: "'Poppins', sans-serif" }}>
                        Manage the daily quest question pool for children.
                    </Typography>
                </Box>
            ...
            >
                <TextField
                    id="daily-quest-search"
                    size="small"
                    placeholder="Search questions…"
                    value={searchTerm}
                    onChange={(e) => { setSearchTerm(e.target.value); setPage(1); }}
                    inputProps={{ name: "daily-quest-search", "aria-label": "Search daily quest questions" }}
                    InputProps={{
                        startAdornment: (
                ...
                <FormControl size="small" sx={{ width: { xs: "100%", sm: "140px" } }}>
                    <Select
                        id="daily-quest-age-group"
                        value={ageGroupFilter}
                        displayEmpty
                        onChange={(e) => handleAgeFilterChange(e.target.value)}
                        inputProps={{ id: "daily-quest-age-group", name: "daily-quest-age-group" }}
                        sx={pillSelectSx}
                ...
                <FormControl size="small" sx={{ width: { xs: "100%", sm: "160px" } }}>
                    <Select
                        id="daily-quest-category"
                        value={categoryFilter}
                        displayEmpty
                        onChange={(e) => { setCategoryFilter(e.target.value); setPage(1); }}
                        inputProps={{ id: "daily-quest-category", name: "daily-quest-category" }}
                        sx={pillSelectSx}
                                    ...
                                    {data?.questions.map((q) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                >
                                                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
                                                        <Stack direction="row" justifyContent="center" spacing={1} sx={{ transition: "0.2s ease" }}>
                                    ...
                                    {(!data?.questions || data.questions.length === 0) && (
                                        <TableRow>
                                            <TableCell colSpan={5} align="center" sx={{ py: 10, border: 0 }}>
                                                <QuestIcon sx={{ fontSize: 38, color: "var(--border-color)", mb: 1, display: "block", mx: "auto" }} />
                                                <Typography sx={{ color: "var(--text-secondary)", fontWeight: 600, mb: 0.5, fontSize: "0.95rem" }}>
                                                    No daily quest questions yet
                                                </Typography>
            ...
            <QuestionModal
                open={isModalOpen}
                initialData={editingQuestion as any}
            ...
            <Fab
                color="primary"
                aria-label="add daily quest question"
                onClick={handleOpenAdd}
                sx={{
