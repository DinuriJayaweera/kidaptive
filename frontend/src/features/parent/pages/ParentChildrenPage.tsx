    ...
    return (
        <Box sx={{ fontFamily: "'Poppins', sans-serif" }}>
            ...
            {loading ? (
                ...
                <Paper elevation={0} sx={{ borderRadius: "16px", p: { xs: 3, md: 5 }, background: "#fff", border: "1px solid #e8ecf1", textAlign: "center" }}>
                    <SchoolIcon sx={{ fontSize: 64, color: "#25AFF4", mb: 2 }} />
                    <Typography variant="h5" sx={{ fontFamily: "'Baloo 2', cursive", fontWeight: 700, mb: 1 }}>Welcome! Let's add your child</Typography>
                    <Typography sx={{ color: "#888", mb: 3, maxWidth: 400, mx: "auto", fontSize: 14 }}>Create a child profile so they can start their Kidaptive learning journey.</Typography>
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate("/parent/children/new")} sx={{ backgroundColor: "#25AFF4", borderRadius: "50px", textTransform: "none", fontWeight: 700, px: 4, py: 1.3, boxShadow: "none", "&:hover": { backgroundColor: "#1e8cc3" } }}>Add a child</Button>
                </Paper>
            ) : (
                    ...
                    {children.map((child) => {
                        ...
                        return (
                            <Grid size={{ xs: 12, sm: 6, md: 4 }} key={child.childId}>
                                <Paper elevation={0} sx={{ borderRadius: "16px", p: 3, background: "#fff", border: "1px solid #e8ecf1", textAlign: "center", transition: "all 0.2s", "&:hover": { transform: "translateY(-4px)", boxShadow: "0 12px 24px rgba(0,0,0,0.06)" }, position: "relative" }}>
                                    {/* Action buttons */}
                                    <Box sx={{ position: "absolute", top: 12, right: 12, display: "flex", gap: 0.5 }}>
                                        <Tooltip title="Edit">
                                            <IconButton size="small" onClick={() => handleEditOpen(child)} sx={{ color: "#94a3b8", "&:hover": { color: "#25AFF4", background: "#e0f2fe" } }}>
                                                <EditIcon sx={{ fontSize: 18 }} />
                                            </IconButton>
                                        </Tooltip>
                                        <Tooltip title="Delete">
                                            <IconButton size="small" onClick={() => handleDeleteOpen(child)} sx={{ color: "#94a3b8", "&:hover": { color: "#ef4444", background: "#fef2f2" } }}>
                                                <DeleteIcon sx={{ fontSize: 18 }} />
                                            </IconButton>
                                        </Tooltip>
                                    </Box>

                                    <Avatar sx={{ width: 64, height: 64, mx: "auto", mb: 2, fontSize: "2rem", backgroundColor: "#e0f2fe" }}>
                                    ...
                                    <Box sx={{ background: "#f8fafc", borderRadius: 2, p: 1.5, mb: 3, display: "flex", justifyContent: "center", gap: 3 }}>
                                        ...
                                        <Box>
                                            <Typography sx={{ fontSize: 11, color: "#94a3b8", fontWeight: 600, textTransform: "uppercase" }}>Gems</Typography>
                                            <Typography sx={{ fontFamily: "'Baloo 2', cursive", fontWeight: 700, color: "#FFCC35", fontSize: 18 }}>{child.gems}</Typography>
                                        </Box>
                                    </Box>

                                    <Button fullWidth onClick={() => navigate(`/parent/child/${child.childId}`)} sx={{ textTransform: "none", fontWeight: 600, borderRadius: "12px", border: "2px solid #e2e8f0", color: "#475569", "&:hover": { border: "2px solid #25AFF4", color: "#25AFF4", background: "transparent" } }}>
                                        View Progress
                                    </Button>
                                </Paper>
                            </Grid>
            ...
            <Dialog open={editOpen} onClose={() => setEditOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
                ...
                <DialogActions sx={{ px: 3, pb: 3 }}>
                    ...
                    <Button
                        ...
                        variant="contained"
                        sx={{ textTransform: "none", backgroundColor: "#25AFF4", borderRadius: "12px", fontWeight: 600, boxShadow: "none", "&:hover": { backgroundColor: "#1e8cc3" } }}
                    >
            ...
            <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
                ...
                <DialogActions sx={{ px: 3, pb: 3 }}>
                    <Button onClick={() => setDeleteOpen(false)} sx={{ textTransform: "none", color: "#6b7280" }}>Cancel</Button>
                    <Button
                        onClick={handleDeleteConfirm}
                        disabled={deleteLoading}
                        variant="contained"
                        sx={{ textTransform: "none", backgroundColor: "#ef4444", borderRadius: "12px", fontWeight: 600, boxShadow: "none", "&:hover": { backgroundColor: "#dc2626" } }}
                    >
