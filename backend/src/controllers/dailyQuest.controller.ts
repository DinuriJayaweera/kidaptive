...
export const getStats = async (_req: Request, res: Response): Promise<void> => {
    try {
        await DailyQuestQuestion.updateMany(
        ...
        console.error('Error fetching daily quest stats:', error);
...
export const getQuestions = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [questions, total] = await Promise.all([
            DailyQuestQuestion.find(query).skip(skip).limit(l).sort({ createdAt: -1 }),
            DailyQuestQuestion.countDocuments(query)
        ]);

        res.json({ questions, total, page: p, pages: Math.ceil(total / l) });
    } catch (error) {
        console.error('Error fetching daily quest questions:', error);
...
export const createQuestion = async (req: Request, res: Response): Promise<void> => {
    try {
        const question = new DailyQuestQuestion(req.body);
        await question.save();
        res.status(201).json(question);
    } catch (error) {
        console.error('Error creating daily quest question:', error);
...
export const updateQuestion = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const question = await DailyQuestQuestion.findByIdAndUpdate(id, req.body, { new: true });
        if (!question) {
        ...
        console.error('Error updating daily quest question:', error);
...
export const deleteQuestion = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const question = await DailyQuestQuestion.findByIdAndDelete(id);
        if (!question) {
            res.status(404).json({ message: 'Question not found' });
            return;
        }
        ...
        console.error('Error deleting daily quest question:', error);
